function avail=avail_contour4(lats, lons, vpl, hpl, emt, sig_acc, isinbnd,percent,vhal,pa_mode,latmax)
%*************************************************************************
...
%   AVAIL_CONTOUR2(LATS, LONS, VPL, HPL)
...
if(pa_mode)
    avail=sum(((vpl <= vhal(1)) & (hpl <= vhal(2)) & (emt <= vhal(3)) & (sig_acc <= vhal(4)))')'/n_times;
else
    avail=sum(((hpl < vhal(2))& (emt <= vhal(3)) & (sig_acc <= vhal(4)))')'/n_times;
end
...
cos_lats(cos_lats < cos(latmax)) = 0;

%idx=find((avail >= percent) & isinbnd);
idx=find((avail >= percent));
if(~isempty(idx))
  %coverage=sum(cos_lats(idx))/sum(isinbnd.*cos_lats);
  coverage=sum(cos_lats(idx))/sum(cos_lats);
...
if(pa_mode)
    bartext = ['VAL = ' num2str(vhal(1)) ',  HAL = ' ...
			   num2str(vhal(2)) ', EMT_t_h = ' num2str(vhal(3)) ...
               ', \sigma_a_c_c = ' num2str(vhal(4)), ...
               ', Coverage(' num2str(percent*100,3) '%) = '...
               num2str(coverage) '%'];
else
    bartext = ['HAL = ' num2str(vhal(2)) ...
